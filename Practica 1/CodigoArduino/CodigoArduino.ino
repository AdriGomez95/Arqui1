...
//para el manjeno de texto dinamico serial
#define S3 A1
...
void setup() {
  ...
  //------------------------Para comuncacion serial----------
  pinMode (S3, INPUT); //Este es para la comunicacion serial
  //-------------------------------------------------
...
void loop() {
  if (digitalRead(S3) == HIGH) {//pregunta si queremmos texto estatico
  ...
  else //aqui va el texto dinamico serial
  {
    //----------------Esto seria texto dinamico
    textoSerial();
  }
}

void mensajeIzquierda()
{
  if (digitalRead(S1) == LOW && digitalRead(S2) == HIGH) {
    ...
    for (int i = 0; i < 200; i++) {
      ...
      if (digitalRead(S1) == HIGH || digitalRead(S1) == LOW && digitalRead(S2) == LOW ) {
        pantalla.borrar();
        break;
      }
      delay(velocidad);
...
void mensajeDerecha()
{
  if (digitalRead(S1) == LOW && digitalRead(S2) == LOW) {
    for (int i = 0; i > -200; i++) {
      ...
      if (digitalRead(S1) == HIGH || digitalRead(S1) == LOW && digitalRead(S2) == HIGH) {
        pantalla.borrar();
        break;
      }
      delay(velocidad);
...
{
  char data[50];
  cadena.toCharArray(data, 50);
  pantalla.borrar();
  for (int i = 0; i > -200; i++) {
    controlVelocidad();
    pantalla.escribirFrase(data, -73+i); // Texto, posicion en la pantalla
    if (digitalRead(S3) == HIGH || i==cadena.length()+100)
    {
      pantalla.borrar();
      break;
    }
...
void mensajeIzq()
{
  ...
  for (int i = 0; i < 200; i++) {
    controlVelocidad();
    pantalla.escribirFrase(data, 16-i);
    if (digitalRead(S3) == HIGH || i==cadena.length()+100)
    {
      pantalla.borrar();
      break;
...
void mensajeMatrix()
{
  while (digitalRead(S1) == HIGH) {
    ...
    for(int i = 0; i <= cadena.length(); i++){
      pantalla.borrar();
      pantalla.escribirCaracter(data[i], 0);
      delay(800);
    }
